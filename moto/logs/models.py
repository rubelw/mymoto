...
class LogGroup:
    ...
    def put_retention_policy(self, retention_in_days):
        print('testing')
        self.retention_policy = int(retention_in_days)
        return None
    ...
    def to_describe_dict(self):
        return {
            ...
            "metricFilterCount": 0,
            "retentionInDays": self.retention_policy,
...
class LogsBackend(BaseBackend):
    ...
    def put_retention_policy(self, log_group_name, retention_in_days):
        if log_group_name not in self.groups:
            raise ResourceNotFoundException()
        self.groups[log_group_name].retention_policy = retention_in_days

    def tag_log_group(self, log_group_name, tags):
        if log_group_name not in self.groups:
            raise ResourceNotFoundException()
        log_group = self.groups[log_group_name]
        sys.stderr.write('log group is: '+str(log_group))
    ...
    def get_log_group_tags(self, log_group_name):
        if log_group_name not in self.groups:
            raise ResourceNotFoundException()
        self.groups[log_group_name].tags

        sys.stderr.write(str(self.groups[log_group_name].tags))
