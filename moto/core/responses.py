...
class BaseResponse(_TemplateEnvironmentMixin):
    ...
    def call_action(self):
        ...
        if action in method_names:
            ...
            if DEBUG:
                logging.debug('responses.py - BaseResponse(_TemplateEnvironmentMixin) call_action - '
                              ' -caller: ' + str(
                    inspect.stack()[1][3]) + "-" + get_linenumber())
                logging.debug('headers: '+str(headers))
                logging.debug('action: '+str(action))
                logging.debug('method_names: '+str(method_names))
            ...
            if isinstance(response, six.string_types):
                ...
                if DEBUG:
                    logging.debug('returning headers: '+str(headers))
                    logging.debug('returning response: ' + str(response))
                    logging.debug('length of response: '+str(len(response)))
                ...
                if DEBUG:
                    logging.debug('returning body: ' + str(body))
                    logging.debug('returning status: '+str(status))
