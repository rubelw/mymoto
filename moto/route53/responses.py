...
class Route53(BaseResponse):
    ...
    def get_change(self, request, full_url, headers):
        ...
        myid = parsed_url.path.rstrip('/').rsplit('/', 1)[1]

        if DEBUG:
            logging.debug("\tid: "+str(myid))
    ...
    def change_tags_for_resource(self, request, full_url, headers):
    ...
    def list_or_change_tags_for_resource_request(self, request, full_url, headers):
        ...
        parsed_url = urlparse(full_url)
        id_ = parsed_url.path.split("/")[-1].split("%2F")[-1]

        type_ = parsed_url.path.split("/")[-2]

        if DEBUG:
            logging.debug('id: '+str(id_))
            logging.debug('type: '+str(type_))

        if request.method == "GET":
            if DEBUG:
                logging.debug('is a get')
            tags = route53_backend.list_tags_for_resource(id_)

            if tags:
                template = Template(LIST_TAGS_FOR_RESOURCE_RESPONSE)
            else:
                template = Template(LIST_TAGS_FOR_RESOURCE_RESPONSE_WITHOUT_TAGS)

            if DEBUG:
                logging.debug('returning: '+str(template.render(
                resource_type=type_, resource_id=id_, tags=tags)))
...
LIST_TAGS_FOR_RESOURCE_RESPONSE_WITHOUT_TAGS = """
<ListTagsForResourceResponse xmlns="https://route53.amazonaws.com/doc/2015-01-01/">
    <ResourceTagSet>
