...
@mock_route53
def test_list_tags_for_resource_request():
    conn = boto3.client('route53', region_name='us-east-1')

    firstzone = conn.create_hosted_zone(Name="testdns.aws.com",CallerReference='12345')

    zones = conn.get_hosted_zone(Id=firstzone['HostedZone']['Id'])
    zone_id = zones['HostedZone']['Id']
    assert zones['HostedZone']['Name'] == 'testdns.aws.com.'
